  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={onClose}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl w-full max-w-2xl animate-scale-in z-50 max-h-[90vh] flex flex-col overflow-hidden">
          {/* Header */}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" id="key-form">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  />
                  {errors.code && <p className="mt-1 text-sm text-accent">{errors.code.message}</p>}
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <select
                    ...
                    className={`
                      ...
                      ${errors.key_type_id ? "border-accent" : "border-gray-200"}
                      focus:border-primary focus:ring-4 focus:ring-primary/10
                      outline-none transition-all
                    `}
                  >
                  ...
                  {errors.key_type_id && <p className="mt-1 text-sm text-accent">{errors.key_type_id.message}</p>}
                </div>
                ...
                {isEmpresarial && (
                  <div className="md:col-span-2">
                    ...
                    <select
                      ...
                      className={`
                        w-full px-4 py-3 rounded-lg border-2
                        ${errors.client_id ? "border-accent" : "border-gray-200"}
                        focus:border-primary focus:ring-4 focus:ring-primary/10
                        outline-none transition-all
                      `}
                    >
                    ...
                    {errors.client_id && <p className="mt-1 text-sm text-accent">{errors.client_id.message}</p>}
                  </div>
                )}
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`
                      w-full px-4 py-3 rounded-lg border-2
                      ${errors.init_date ? "border-accent" : "border-gray-200"}
                      focus:border-primary focus:ring-4 focus:ring-primary/10
                      outline-none transition-all
                    `}
                  />
                  {errors.init_date && <p className="mt-1 text-sm text-accent">{errors.init_date.message}</p>}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`
                      w-full px-4 py-3 rounded-lg border-2
                      ${errors.due_date ? "border-accent" : "border-gray-200"}
                      focus:border-primary focus:ring-4 focus:ring-primary/10
                      outline-none transition-all
                    `}
                  />
                  {errors.due_date && <p className="mt-1 text-sm text-accent">{errors.due_date.message}</p>}
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <select
                    ...
                    className={`
                      w-full px-4 py-3 rounded-lg border-2
                      ${errors.state ? "border-accent" : "border-gray-200"}
                      focus:border-primary focus:ring-4 focus:ring-primary/10
                      outline-none transition-all
                    `}
                  >
                  ...
                  {errors.state && <p className="mt-1 text-sm text-accent">{errors.state.message}</p>}
                </div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div className="grid grid-cols-2 gap-3 p-4 bg-gray-50 rounded-lg border-2 border-gray-200">
                      {permissions.map((permission) => (
                        <label
                          key={permission.id}
                          className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 hover:border-primary cursor-pointer transition-all"
                        >
                          <input
                            type="checkbox"
                    ...
                    {isEmpresarial && selectedPermissions.length === 0 && (
                      <p className="mt-2 text-sm text-amber-600">Se recomienda seleccionar al menos un permiso</p>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-3 rounded-lg border-2 border-gray-200 text-gray-700 font-medium hover:bg-gray-100 transition-colors disabled:opacity-50"
              >
                Cancelar
              </button>
              <button
                ...
                form="key-form"
                disabled={isSubmitting}
                className="flex-1 px-4 py-3 rounded-lg bg-gradient-to-r from-primary to-primary-600 text-white font-medium hover:from-primary-600 hover:to-primary-700 shadow-lg shadow-primary/30 transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {isSubmitting ? (
                  <>
